...
const extractStructuredContent = (content: string, existingSources?: Source[]) => {
  ...
  // Extract sources from the content using regex
  const sourceRegex = /Source:\s*(.*?)\s*–\s*(.*?)(?=\n|$)/g;
  let sourceMatch;
  while ((sourceMatch = sourceRegex.exec(content)) !== null) {
    extractedSources.push({
      type: sourceMatch[1].toLowerCase().replace(/\s+/g, '_'),
      title: sourceMatch[2],
    });
  }

  // Remove source lines from content
  cleanContent = cleanContent.replace(sourceRegex, '');

  // Extract disclaimer
  const disclaimerRegex = /This is informational and not a substitute for formal legal advice\. Consult a qualified Pakistani lawyer for specific cases\./;
  const disclaimerMatch = cleanContent.match(disclaimerRegex);
  if (disclaimerMatch) {
    notes.push(disclaimerMatch[0]);
    cleanContent = cleanContent.replace(disclaimerRegex, '');
  }
...
export function MessageBubble({ message }: MessageBubbleProps) {
  ...
  const finalContent = (cleanContent || message.content || '').trim();
