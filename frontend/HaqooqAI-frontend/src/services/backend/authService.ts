...
class AuthService {
  ...
  /**
   * Validate GitHub token with backend (with retry logic + schema mismatch detection)
   */
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const user: User = {
          ...
          groq_api_key_present: response.data.quota?.has_api_key || false,
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        }

        this.storeUser(user)
        return { user, quota: response.data.quota }
      } catch (error) {
        console.error(
          `Token validation attempt ${attempt}/${maxRetries} failed:`,
          error
        )
        ...
        if (axios.isAxiosError(error)) {
          ...
          // Detect schema mismatch in backend
          if (status === 502) {
            const errorData = error.response?.data
            if (typeof errorData === 'object' && errorData?.detail) {
              if (
                errorData.detail.includes('api_key_hash') ||
                errorData.detail.includes('does not exist')
              ) {
                throw new Error(
                  'Backend database schema needs update - please contact support'
                )
              }
            }
          }
  ...
  async deleteApiKey(userGithubId: number): Promise<void> {
    ...
    await axios.delete(`${BACKEND_URL}${API_ENDPOINTS.DELETE_API_KEY}`, {
