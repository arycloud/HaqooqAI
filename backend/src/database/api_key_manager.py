...
class APIKeyManager:
    ...
    async def store_api_key(self, user_id: int, provider: str, api_key: str) -> bool:
        ...
        try:
            ...
            data = {
                "github_id": user_id,
                "provider": provider,
                "encrypted_key": encrypted_key,  # New column name
    ...
    async def get_api_key(self, user_id: int, provider: str) -> Optional[str]:
        ...
        """
        try:
            # Query the database for the new column structure
            result = self.db.client.table("api_keys").select("encrypted_key, provider").eq(
                "github_id", user_id
    ...
    async def get_user_providers(self, user_id: int) -> List[ProviderStatus]:
        ...
        """
        try:
            # Get all API keys for the user with new structure
